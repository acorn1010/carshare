...
# Everything, including the store integration tests against a real Postgres.
# Start one with scripts/dev_db.sh first.
test-sql:
	$(GO) vet ./...
	CARSHARE_TEST_DATABASE_URL="$(TEST_DB_URL)" $(GO) test -race -count=1 ./...
